; AHKhttp.ahk
; from https://www.autohotkey.com/boards/viewtopic.php?t=4890&start=40
...
HttpHandler(sEvent, iSocket = 0, sName = 0, sAddr = 0, sPort = 0, ByRef bData = 0, bDataLength = 0) {
    ...
    if (!sockets[iSocket]) {
        sockets[iSocket] := new SocketClass(iSocket)
...
class HttpResponse
{
  ...
  __Delete(){
    ...
    Generate() {
        ...
        this.buffer := new BufferClass((StrLen(headers) * 2) + length)
    ...
    SetBody(ByRef body, length) {
        this.body := new BufferClass(length)
        this.body.Write(&body, length)
        this.headers["Content-Length"] := length
    }

    SetBodyText(text) {
        this.body := BufferClass.FromString(text)
...
class BufferClass
{
  ...
  __Delete(){
    ...
    FromString(str, encoding = "UTF-8") {
        length := BufferClass.GetStrSize(str, encoding)
        this.buffer := new BufferClass(length)
