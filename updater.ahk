...
guiMainWidthDefault := 800
guiMainHeightDefault := 350
...
preferedPathDefault := "C:\jvrks\" . targetAppnameLower . "\"
...
targetAppnameLower := StrLower(targetAppname)
if (eq(preferedPath,""))
  preferedPath := "C:\jvrks\" . targetAppnameLower . "\"
...
; first check if there is a new updater version, download if found
if (runMode){
  updateUpdater(false)
}
...
if (!runMode){
  msgDefault := "Please enter or select an installation-directory and press th button ""Use this installation-directory"" then!"
}
...
updateUpdater(force := true){
  global appVersionRemote
  global app
  global bit
  global appnameLower
  global appVersion
  global downloadFileAddExtension
  global appVersionFile
  global extension
  global serverApp
  global serverURLExtension
  global restartBatch
  global longOperationRunning
  ...
  if (bit == "64"){
    appnameLowerWithExtension := appnameLower . extension . downloadFileAddExtension
    url := serverApp . appnameLowerWithExtension
  } else{
    appnameLowerWithExtension := appnameLower . bit . extension . downloadFileAddExtension
    url := serverApp . appnameLowerWithExtension
  }
...
guiMainGuiSize(){
  ...
  global ProgressBar
  global ProgressN
  global progressDownLoadURL
  global KB
  global Text1
  global preferedPathLabel
  global preferedPathText
  global Button1
  global selectFolder
  global setPreferedPathToDefault
  global openFileManager
  global insertFromClipboard
  global showHelp
...
readIni(){
  ...
  global serverURL
  global serverURLExtension
  global serverUpdaterURL
  global serverUpdaterURLExtension
  global CR
  global gui_position
  ...
  global preferedPath
  global targetAppnameLower
  
  IniRead, gui_position, updater_gui.ini, window position, gui_position, Center w%guiMainWidthDefault% h%guiMainHeightDefault%
  IniRead, guiMainWidth, updater_gui.ini, window size, guiMainWidth, %guiMainWidthDefault%
  IniRead, guiMainHeight, updater_gui.ini, window size, guiMainHeight, %guiMainHeightDefault%
...
mainWindow() {
  ...
  ; Menu, MainMenu,Add,Update %targetAppname%, startDownload
  ...
  yMargin := 100
  lv1Width := guiMainWidth - 10
  lv1Height := guiMainHeight - yMargin
  ...
  if (!runMode){
    ...
    Gui, guiMain:Add, Edit, x+m yp-2 r1 w400 VpreferedPathLabel GsetPreferedPath, %preferedPath%
    Gui, guiMain:Add, Button, x+m yp+2 VselectFolder GselectFolder, Select
    Gui, guiMain:Add, Button, x+m yp VsetPreferedPathToDefault GsetPreferedPathToDefault, Default
    Gui, guiMain:Add, Button, x+m yp VopenFileManager GopenFileManager, FM
    ...
    Gui, guiMain:Add, Button, x+m yp vButton1 GstartDownload, Use this installation-directory
  } else {
    Gui, guiMain:Add, Text, xm yp+20 VpreferedPathText, Installation-directory: 
    Gui, guiMain:Add, Edit, x+m yp-2 r1 w400 VpreferedPathLabel GsetPreferedPath, %preferedPath%
    Gui, guiMain:Add, Button, x+m yp+2 vButton1 GstartDownload, Start installation / update
  }
  ...
  if (!runMode){
    if (!eq(preferedPath, "")){
      if (!eq(preferedPath, wrkDir)) {
        if (bit == "64"){
          appnameLowerWithExtension := appnameLower . extension
        } else{
          appnameLowerWithExtension := appnameLower . bit . extension
        }
    ...
    showMessage("Operation DOWNLOAD takes a while! (beeps 3 times if ready!)")
    ...
    if (!FileExist("updater_runMode.txt")){
      FileAppend,This file is created by Updater to select the runMode and to log updates!`n,updater_runMode.txt
    } else {
    ...
    run,%targetAppnameLower%%extension%
...
;------------------------------ showHintColored ------------------------------
showHintColored(handle, s, n, fg, bg, font := "Calibri", fontsize := 10){
...
exit() {
  ...
  if (hMain > 0){
    WinGetPosEx(hMain,gui_x,gui_y,gui_w,gui_h,Offset_X,Offset_Y,Offset_Right,Offset_Bottom)

    dpiScaleW := 96/A_ScreenDPI
    dpiScaleH := 88/A_ScreenDPI
    
    gui_x := gui_x - Offset_X
    gui_y := gui_y - Offset_Y
    gui_w := gui_w - 0
    gui_h := gui_h - 0
    
    mainWidth := round(gui_w * dpiScaleW)
    mainHeight := round(gui_h * dpiScaleH)
    
    IniWrite, x%gui_x% y%gui_y% w%mainWidth% h%mainHeight%, updater_gui.ini, window position, gui_position
    IniWrite, %mainWidth%, updater_gui.ini, window size, guiMainWidth
    IniWrite, %mainHeight%, updater_gui.ini, window size, guiMainHeight

    if (downloadStarted && !hasFinished)
      msgbox, Download aborted!`n`nTemporary files (*.default) are not deleted!
